 ...
 *  Code: https://dejoe.github.com/resizeToWindowHeight
 *  Author: dejoe.tumblr.com
 * 
 */
(function($){
	$.fn.resizeToWindowHeight = function(options,callbackMethod,resizeCallback){
		
		function resizeMe(elem){
			var $this = $(elem);
			var elemGutter = parseFloat($this.css("margin-top")) + parseFloat($this.css("margin-bottom"));
			var elemLength =  elemGutter + $this.height();
			var windowHeight = $(window).height();
			var elemOrigHeight = $this.height("auto")
			if ($this.attr("data-origHeight") > windowHeight){
				$(elem).css("height",(windowHeight - elemGutter) + "px");
			}
		}
		
		return this.each(function(){
			var elem = this;
			$(this).attr("data-origHeight",$(this).height());
			resizeMe(elem);
			callbackMethod.call();
			
			$(window).bind('resize.resizeToWindowHeight',function(){
				resizeMe(elem);
				//If there is a way to resize the scroll bar then call it
				resizeCallback.call();
			});
